...
public class Filters {
	...
	public static String negateFilter(String path,boolean generatedFolder) {
		String outputPath = "";
		if (generatedFolder) {
			outputPath = path.replace(".avi", "_neg.avi");
		}
		else outputPath = path.replace(".mp4", "_neg.avi").replace("videos/", "generatedVideos/");
		String cmd = "ffmpeg -y -i " + path + " -vf lutrgb=r=negval:g=negval:b=negval -c:a ac3 " + outputPath;
		try {
			rt.exec(cmd).waitFor();
	...
	public static String blackWhiteFilter(String path, boolean generetedFolder) {
		String outputPath = "";
		if (generetedFolder) {
			outputPath = path.replace(".avi", "_bw.avi");
		}
		else outputPath = path.replace(".mp4", "_bw.avi").replace("videos/", "generatedVideos/");
		//String cmd = "ffmpeg -y  -i  " + path + " -pix_fmt gray -vcodec rawvideo " + outputPath;
		String cmd = "ffmpeg -y  -i  " + path + " -vf hue=s=0 -c:a ac3 " + outputPath;
		try {
			rt.exec(cmd).waitFor();
		} catch (IOException e) {
			// TODO Auto-generated catch block
	...
	public static String flipFilter(String path, String type, boolean generatedFolder) {
		String outputPath = "";
		if (generatedFolder) {
			outputPath = path.replace(".avi", "_"+type.substring(0, 1)+"flip.avi");
		}
		else outputPath = path.replace(".mp4", "_"+type.substring(0, 1)+"flip.avi").replace("videos/", "generatedVideos/");
	
		String cmd = "";
		if (type.equals("h") || type.equals("horizontal")) {
			cmd = "ffmpeg -i " + path + " -vf hflip " + outputPath;
		} else if (type.equals("v") || type.equals("vertical")) {
			cmd = "ffmpeg -i " + path + " -vf vflip  " + outputPath;
		} else {
	...
	public static String addText(String path, VideoText args, boolean generatedFolder) {
		if (args != null) {
			String outputPath = "";
			if (generatedFolder) {
				outputPath = path.replace(".avi", "_text.avi");
			}
			else outputPath = path.replace(".mp4", "_text.avi").replace("videos/", "generatedVideos/");
